            ...
            ::coder::array<double, 2U> &J)
{
  ...
  array<double, 1U> J_data;
  array<double, 1U> d_J_data;
  ...
  if (J.size(0) == 1) {
    ...
    for (pInd = 0; pInd < numPixels; pInd++) {
      ...
      J[pInd] = std::fmin(internal::maximum(c_J_data, J_size), b_I[pInd]);
    }
    ...
    for (pInd = numPixels; pInd >= 1; pInd--) {
      ...
      J[pInd - 1] =
          std::fmin(internal::maximum(c_J_data, J_size), b_I[pInd - 1]);
      ind = 0;
      exitg1 = false;
    ...
    for (pInd = 0; pInd < numPixels; pInd++) {
      ...
      J[pInd] = std::fmin(internal::maximum(J_data), b_I[pInd]);
    }
    locationStack.set_size(1, numPixels << 1);
    stackTop = -1;
    for (pInd = numPixels; pInd >= 1; pInd--) {
      ...
      J[pInd - 1] = std::fmin(internal::maximum(d_J_data), b_I[pInd - 1]);
      ind = 0;
      exitg1 = false;
      while ((!exitg1) && (ind <= trueCount - 1)) {
        if ((J[imnhInds_data[ind] - 1] < J[pInd - 1]) &&
